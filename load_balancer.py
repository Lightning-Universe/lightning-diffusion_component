...
def calls(work: LightningWork):
    ...
    return [
        ...
        for call_hash in work._calls.keys()
        if call_hash != CacheCallsKeys.LATEST_CALL_HASH
    ]
...
def oldest_called(running_works: List[LightningWork]) -> LightningWork:
    ...
    return timestamp_of_most_recent_call[
        sorted(timestamp_of_most_recent_call.keys())[-1]
    ]
...
class LoadBalancer(LightningFlow):
    ...
    def __init__(
        ...
        self._work_attribute_prefix = (
            f"{self._name}_" if self.name != "" else DEFAULT_WORK_ATTRIBUTE_PREFIX
        )
    ...
    def pool(self):
        ...
        return [
            work
            for work in self._work_pool.values()
    ...
    def run(self, *args, **kwargs):
        with self._work_pool_rw_lock:
            ...
            running = [
                work
                for work in self.pool
                if work.status.stage == WorkStageStatus.RUNNING
            ]
            available = [
                work
                for work in running
                if pending_calls(work) < self.max_pending_calls_per_work
            ]
            pending = [
                work
                for work in self.pool
                if work.status.stage
                in [WorkStageStatus.PENDING, WorkStageStatus.NOT_STARTED]
            ]
            ...
            if len(available) > 0:
                ...
                logger.info(
                    f"Found running Work ({oldest_called_work.name}), calling `run()`"
                )
            ...
            if len(pending) == 0:
                ...
                if len(running) == 0:
                    logger.info(
                        f"No other Works are running, calling `run()` on new Work ({new_work.name})"
                    )
            ...
            if len(running) > 0:
                current_pending_calls = [
                    (work, pending_calls(work)) for work in running
                ]
                least_overloaded = min(current_pending_calls, key=lambda work: work[1])
                logger.info(
                    f"Found least overloaded running Work ({least_overloaded[0].name}), calling `run()`"
                )
                least_overloaded[0].run(*args, **kwargs)
            else:
                current_pending_calls = [
                    (work, pending_calls(work)) for work in pending
                ]
                least_overloaded = min(current_pending_calls, key=lambda work: work[1])
                logger.info(
                    f"Found least overloaded pending Work ({least_overloaded[0].name}), calling `run()`"
                )
                least_overloaded[0].run(*args, **kwargs)
